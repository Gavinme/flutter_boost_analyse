...
class BoostNavigator {
  ...
  Future<T> push<T extends Object?>(String name,
    ...
    bool isFlutter = isFlutterPage(name);//通过 Factory 判断是否存在 name 该页面
    if (isFlutter && withContainer) {
      //页面在 pageRouter.dart 中注册，且打开新的 Flutter 容器，需要通过 hero 模拟打开时转场动画
      ...
      // 页面在 pageRouter.dart 中未注册，
      // 可能是一个原生页面，
      // 不新开 Flutter 容器，满足以上条件时
